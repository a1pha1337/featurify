...
class ApiExceptionHandler : ResponseEntityExceptionHandler() {
    ...
    fun integrity(ex: DataIntegrityViolationException, request: HttpServletRequest) =
        problem(HttpStatus.CONFLICT, "CONFLICT", "The operation conflicts with the current resource state", request.requestURI)

    @ExceptionHandler(DomainValidationException::class)
    fun domainValidation(ex: DomainValidationException, request: HttpServletRequest) =
        problem(HttpStatus.BAD_REQUEST, "VALIDATION_ERROR", ex.message ?: "Request validation failed", request.requestURI,
    ...
    fun unavailable(ex: Exception, request: HttpServletRequest): ResponseEntity<Any> {
        log.error("API storage temporarily unavailable", ex)
        return problem(HttpStatus.SERVICE_UNAVAILABLE, "SERVICE_UNAVAILABLE", "Service temporarily unavailable", request.requestURI)
    }
    ...
    override fun handleExceptionInternal(ex: Exception, body: Any?, headers: HttpHeaders,
                                         statusCode: HttpStatusCode, request: WebRequest): ResponseEntity<Any>? {
        if ((request as ServletWebRequest).response?.isCommitted == true) return null
        val details = when (ex) {
            is MethodArgumentNotValidException -> ex.bindingResult.allErrors.map {
                ValidationDetail((it as? FieldError)?.field ?: it.objectName, it.defaultMessage ?: "invalid")
            }
            is MissingServletRequestParameterException -> listOf(ValidationDetail(ex.parameterName, "must be provided"))
            else -> emptyList()
        }
    ...
    fun unexpected(ex: Exception, request: HttpServletRequest): ResponseEntity<Any> {
        ...
        return problem(HttpStatus.INTERNAL_SERVER_ERROR, "INTERNAL_ERROR", "An unexpected error occurred", request.requestURI)
    }

    private fun problem(status: HttpStatusCode, code: String, detail: String, path: String,
                        details: List<ValidationDetail> = emptyList(), headers: HttpHeaders = HttpHeaders()): ResponseEntity<Any> =
        ResponseEntity.status(status).headers(headers).contentType(MediaType.APPLICATION_PROBLEM_JSON)
