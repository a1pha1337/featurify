...
class NamespaceTokenFilter(private val tokens: AccessTokenService, private val mapper: ObjectMapper) : OncePerRequestFilter() {
    override fun doFilterInternal(request: HttpServletRequest, response: HttpServletResponse, chain: FilterChain) {
        ...
        val namespace = try {
            ...
            ApiProblems.write(mapper, request, response, HttpStatus.SERVICE_UNAVAILABLE,
                "SERVICE_UNAVAILABLE", "Authentication temporarily unavailable")
            return
        }
        if (namespace == null) {
        ...
        context.authentication = UsernamePasswordAuthenticationToken(
            NamespacePrincipal(namespace.id!!, namespace.key), null, listOf(SimpleGrantedAuthority("NAMESPACE_READ")),
        )
