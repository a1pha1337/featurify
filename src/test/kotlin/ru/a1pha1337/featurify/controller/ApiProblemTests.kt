...
class ApiProblemTests {
    ...
    private val mvc = MockMvcBuilders.standaloneSetup(AccessTokenController(tokens), AdminFeatureController(mock(FeatureToggleService::class.java)))
    ...
    fun `invalid JSON and bean validation produce RFC9457 without rejected values`() {
        mvc.perform(post(path).contentType(MediaType.APPLICATION_JSON).content("{"))
            ...
            .andExpect(jsonPath("$.instance").value(path)).andExpect(jsonPath("$.message").doesNotExist())
    ...
    fun `MVC type mismatch unsupported method and content type use proper client error statuses`() {
        mvc.perform(delete("$path/not-a-uuid"))
            .andExpect(status().isBadRequest).andExpect(jsonPath("$.status").value(400))
        mvc.perform(put(path)).andExpect(status().isMethodNotAllowed).andExpect(header().exists("Allow"))
            .andExpect(jsonPath("$.code").value("METHOD_NOT_ALLOWED"))
        mvc.perform(post(path).contentType(MediaType.TEXT_PLAIN).content("test"))
