...
class AccessTokenControllerTests {
    ...
    private val mvc = MockMvcBuilders.standaloneSetup(AccessTokenController(service))
    ...
    fun `creation returns secret once with no store cache policy`() {
        ...
        mvc.perform(post("/api/v1/namespaces/blue/tokens").contentType(MediaType.APPLICATION_JSON).content("""{"name":"backend"}"""))
            .andExpect(status().isCreated).andExpect(header().string("Cache-Control", "no-store"))
            .andExpect(jsonPath("$.token").value("test-only-secret")).andExpect(jsonPath("$.tokenHash").doesNotExist())
    }
    ...
    fun `list excludes secrets and hashes and revoke uses namespace scope`() {
        ...
        mvc.perform(get("/api/v1/namespaces/blue/tokens"))
            .andExpect(status().isOk).andExpect(jsonPath("$[0].name").value("backend"))
            .andExpect(jsonPath("$[0].token").doesNotExist()).andExpect(jsonPath("$[0].tokenHash").doesNotExist())
    ...
    fun `blank token name is rejected before service call`() {
        mvc.perform(post("/api/v1/namespaces/blue/tokens").contentType(MediaType.APPLICATION_JSON).content("""{"name":" "}"""))
