...
class FeatureSecurityDatabaseTests {
    ...
    fun setup() {
        ...
        features.createFeature(blue.key, CreateFeatureRequest(key = "enabled", type = FeatureType.BOOLEAN, booleanValue = false))
        features.createFeature(red.key, CreateFeatureRequest(key = "enabled", type = FeatureType.BOOLEAN, booleanValue = true))
    ...
    fun `read scope is always token namespace and query cannot select another namespace`() {
        mvc.perform(get("/api/v1/features").header("Authorization", "Bearer ${blueToken.token}"))
            .andExpect(status().isOk).andExpect(jsonPath("$.content[0].value").value(false))
        mvc.perform(get("/api/v1/features/enabled").header("Authorization", "Bearer ${redToken.token}"))
            .andExpect(status().isOk).andExpect(jsonPath("$.value").value(true))
        mvc.perform(get("/api/v1/features:resolve").param("keys", "enabled").param("namespace", blue.key)
            .header("Authorization", "Bearer ${blueToken.token}"))
            .andExpect(status().isOk).andExpect(jsonPath("$.features.enabled.value").value(false))
        for (path in listOf("/api/v1/features", "/api/v1/features/enabled", "/api/v1/features:resolve?keys=enabled")) {
            mvc.perform(get(path).param("namespace", red.key).header("Authorization", "Bearer ${blueToken.token}"))
    ...
    fun `revoked tokens and inactive or deleted namespaces are rejected immediately`() {
        ...
        mvc.perform(get("/api/v1/features").header("Authorization", "Bearer ${redToken.token}"))
            .andExpect(status().isUnauthorized)
    ...
    fun `malformed duplicated and oversized credentials are rejected`() {
        ...
        mvc.perform(get("/api/v1/features").header("Authorization", "Bearer ${blueToken.token}", "Bearer ${redToken.token}"))
            .andExpect(status().isUnauthorized)
    }
    ...
    fun `all admin routes reject namespace tokens and anonymous requests`() {
        val requests = listOf(
            post("/api/v1/features"), patch("/api/v1/features/enabled"), delete("/api/v1/features/enabled"),
            patch("/api/v1/features/enabled/group"), get("/api/v1/features/enabled/history"),
            post("/api/v1/namespaces"), get("/api/v1/namespaces"), delete("/api/v1/namespaces/${blue.key}"),
            post("/api/v1/groups"), get("/api/v1/groups"), delete("/api/v1/groups/checkout"),
            post("/api/v1/namespaces/${blue.key}/tokens"), get("/api/v1/namespaces/${blue.key}/tokens"),
            delete("/api/v1/namespaces/${blue.key}/tokens/${blueToken.id}"),
        )
        for (request in requests) {
            mvc.perform(request).andExpect(status().isUnauthorized)
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
            mvc.perform(request.header("Authorization", "Bearer ${blueToken.token}"))
    ...
    fun `Keycloak authentication permits administration and OAuth2 login session still works`() {
        mvc.perform(post("/api/v1/features").param("namespace", blue.key).with(jwt())
            .contentType(MediaType.APPLICATION_JSON).content("""{"key":"newFlag","type":"BOOLEAN","booleanValue":true}"""))
            .andExpect(status().isCreated)
        mvc.perform(get("/api/v1/features/enabled/history").param("namespace", blue.key).with(jwt()))
            .andExpect(status().isOk)
        mvc.perform(get("/api/v1/namespaces/${blue.key}/tokens").with(oauth2Login()))
            .andExpect(status().isOk)
    }

    @Test
    fun `REST errors use problem details after namespace authentication`() {
        mvc.perform(get("/api/v1/features/missing").header("Authorization", "Bearer ${blueToken.token}"))
        ...
        mvc.perform(get("/api/v1/features:resolve").header("Authorization", "Bearer ${blueToken.token}"))
            .andExpect(status().isBadRequest).andExpect(jsonPath("$.details[0].field").value("keys"))
        mvc.perform(get("/api/v1/features").param("query", "a").header("Authorization", "Bearer ${blueToken.token}"))
            .andExpect(status().isBadRequest).andExpect(jsonPath("$.details[0].field").value("query"))
        mvc.perform(get("/api/v1/unknown").with(jwt()))
            .andExpect(status().isNotFound).andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_PROBLEM_JSON))
            .andExpect(jsonPath("$.code").value("NOT_FOUND"))
        mvc.perform(put("/api/v1/features").with(jwt()))
