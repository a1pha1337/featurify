...
- Для PAYLOAD поле `payloadValue` — строка с полным JSON-документом (до 65536 символов).
  `Managed` заменяет весь документ из YAML, `InitialOnly` задаёт его только при создании;
  дальнейшие ручные изменения сохраняются. Пробелы и порядок ключей объектов не меняют значение.
...
Пример JSON-конфигурации в `spec.features` или `spec.groups[].features`:

```yaml
- key: checkout-config
  type: PAYLOAD
  payloadValue: |
    {"timeoutMs":1500,"theme":{"accent":"blue"},"methods":["card","wire"]}
```

Допустимы объект, массив, строка, число, boolean и JSON `null` (в YAML: `payloadValue: 'null'`).
Поле обязательно только для PAYLOAD; поля значений других типов запрещены.
CRD проверяет наличие, тип и длину строки; сервер проверяет JSON, включая дубли ключей,
до изменения данных. Ошибка JSON даёт `Ready=False / InvalidManifest` и откатывает весь apply.
При обновлении оператора сначала примените обновлённую `deploy/crd.yaml` и обновите Featurify.
Пример полного CR находится в `examples/checkout.yaml`.
