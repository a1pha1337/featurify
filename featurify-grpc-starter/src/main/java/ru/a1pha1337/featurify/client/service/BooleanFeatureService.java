...
/**
 * Blocking, thread-safe access to boolean features, with a per-service response cache.
 ...
 */
public class BooleanFeatureService {
    private final FeaturifyClient client;
    private final Cache<GetFeatureRequest, BooleanFeatureResponse> cache;
    private final String defaultGroup;

    public BooleanFeatureService(FeaturifyClient client, Duration ttl, long maximumSize) {
        this(client, ttl, maximumSize, null);
    }

    public BooleanFeatureService(FeaturifyClient client, Duration ttl, long maximumSize, String defaultGroup) {
        this(client, ttl, maximumSize, defaultGroup, Ticker.systemTicker());
    }

    BooleanFeatureService(FeaturifyClient client, Duration ttl, long maximumSize, String defaultGroup, Ticker ticker) {
    ...
    public boolean isEnabled(String key) {
        return isEnabled(key, defaultGroup);
    }

    public boolean isEnabled(String key, String group) {
        return getFeature(key, group).getValue();
    }

    public BooleanFeatureResponse getFeature(String key) {
     ...
     */
    public BooleanFeatureResponse getFeature(String key, String group) {
        GetFeatureRequest cacheKey = GetFeatureRequest.newBuilder().setKey(key)
                .setGroup(group == null ? "" : group).build();
        return cache.get(cacheKey, request -> client.getBooleanFeature(request.getKey(), request.getGroup()));
    }
}
